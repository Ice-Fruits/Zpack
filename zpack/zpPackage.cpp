							...
							u32* packSize, u32* flag, u32* availableSize, u64* contentHash) const
{
	...
	if (filenameBuffer != NULL)
	{
		...
		//memcpy(filenameBuffer, m_filenames[index].c_str(), filenameBufferSize);
...
bool Package::defrag(Callback callback, void* callbackParam)
{
	...
	fseeko64(tempFile, sizeof(m_header), SEEK_SET);
	...
	for (u32 i = 0; i < fileCount; ++i)
	{
			...
			|| currentChunkSize > MIN_CHUNK_SIZE)
		{
			if (currentChunkSize > 0)
			{
				...
				fseeko64(m_stream, currentChunkPos, SEEK_SET);
...
{
	fseeko64(m_stream, 0, SEEK_END);
	u64 packageSize = ftello64(m_stream);
...
bool Package::readFileEntries()
{
	...
	fseeko64(m_stream, m_header.fileEntryOffset, SEEK_SET);
...
bool Package::readFilenames()
{
	...
	fseeko64(m_stream, m_header.filenameOffset, SEEK_SET);
...
{
	fseeko64(m_stream, entry.byteOffset, SEEK_SET);
