...
bool ZpExplorer::extractRecursively(ZpNode* node, zp::String externalPath, zp::String internalPath)
{
	...
	if (!internalPath.empty())	//in case extract the root directory
	{
		...
		if (!isDirectoryExistInternal(externalPath))
		{
			return false;
		}
	}
...
{
	size_t pos = name.find_last_of(DIR_STR);
	if (pos == zp::String::npos)
	{
...
bool ZpExplorer::createDirectory(const zp::String & dirPath)
{
#ifdef WIN32
...
bool ZpExplorer::isDirectoryExistInternal(const zp::String & dirPath)
{
#ifdef WIN32
	WIN32_FIND_DATA fd;
