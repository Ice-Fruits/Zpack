...
{
	CFileDialog dlg(FALSE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("zpack archives (*.npk)|*.npk|All Files (*.*)|*.*||"));
	if (dlg.DoModal() != IDOK)
	{
		return;
	}
...
{
	CFileDialog dlg(TRUE, NULL, NULL, 0, _T("zpack files (*.npk)|*.npk|All Files (*.*)|*.*||"));
	if (dlg.DoModal() != IDOK)
	{
		return;
	}
...
void CMainFrame::OnDropFiles(HDROP hDropInfo)
{
	...
	if (nFiles == 1)
	{
		//if only 1 file with .npk extension, open it as a package
